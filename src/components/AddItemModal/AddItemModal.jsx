...
const AddItemModal = ({activeModal, closeActiveModal, handleAddItem}) => {
    ...
    return <ModalWithForm buttonText={"Add garment"} title={"Add garment"} isOpen={activeModal === "add-garment"} handleCloseClick={closeActiveModal} handleFormSubmit={handleAddItem} formData={{clothName, clothImageURL, clothWeatherType}} submitButtonClass={submitButtonClass}>
        <label htmlFor="name" className="modal__label">
            Name <input type="text" className="text__input modal__input" id="name" placeholder="Name" onChange={(e)=> setClothName(e.target.value)} required/>
        </label>
        <label htmlFor="imageUrl" className=" modal__label">
            Image {" "} <input type="text" className="text__input input__image" id="imageUrl" placeholder="Image URL" onChange={(e)=> setClothImageURL(e.target.value)} required/>
        </label>
